...
var promptQuestions = {
  ...
  generate_coffee: {
    type: "confirm",
    name: "generateProblem",
    message: "Generate coffeescript file for problem 2?",
    default: false
    },
...
var solveHandlerSuccess = function(problem){
...
var generateFileHandler = function(problem, suffix){
  var question, confirmation;
  if (suffix === 'js'){question = [promptQuestions.generate_js]; confirmation = [promptQuestions.generate_js_overwrite];}
  else {question = [promptQuestions.generate_coffee]; confirmation = [promptQuestions.generate_coffee_overwrite];}
...
if ( program.solve    || program.solve === 0) problemNumberHandler(solveHandlerSuccess, noFilesFailure, program.solve );

if ( program.generate || program.generate === 0){
  if ( program.coffee || program.coffee === 0){
    if (typeof program.coffee === 'number')  problemNumberHandler(generateFileHandler, noFilesFailure, program.coffee, 'coffee');
    else problemNumberHandler(generateFileHandler, noFilesFailure, program.generate, 'coffee');
  } else {
    problemNumberHandler(generateFileHandler, noFilesFailure, program.generate, 'js');
  }
} 
